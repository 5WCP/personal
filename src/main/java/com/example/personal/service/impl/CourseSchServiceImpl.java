...
public class CourseSchServiceImpl implements CourseSchService{
	...
	public CourseSchResponse addCourse(CourseSchRequest request) {
		...
		Cou.setEndTime(enTi);
		int classtime = (int)ChronoUnit.SECONDS.between(stTi, enTi); // 上課時間
		if((Cou.getCredits() * 3600) != classtime) { // 上課時間比對學分數
	...
	public CourseSchResponse getCourse(CourseSchRequest request) {
		...
		if(!StringUtils.hasText(reqCo)) {
			return new CourseSchResponse("預搜尋的課程代碼請確實填寫");
		}
	...
	public CourseSchResponse deleteCourse(CourseSchRequest request) {
		...
		CourseSch checkCou = courseSchDao.findById(Cou.getCourseCode()).get();
		if(!Cou.getCourseName().equals(checkCou.getCourseName())
			|| Cou.getCredits() != checkCou.getCredits()
			|| !Cou.getTakeClassDay().equals(checkCou.getTakeClassDay())
			|| !Cou.getStartTime().equals(checkCou.getStartTime())
			|| !Cou.getEndTime().equals(checkCou.getEndTime())) {
			return new CourseSchResponse("刪除失敗 預刪除的課程資料與資料庫中的資料不符");
		}
		if(checkCou.getStuCount() > 0) {
			return new CourseSchResponse("刪除失敗 此課程有" + checkCou.getStuCount() + "名學生選修中");
		}
		courseSchDao.delete(checkCou);
		return new CourseSchResponse(checkCou , "課程刪除成功");
	}
	...
	public CourseSchResponse findCourseName(CourseSchRequest request) {
		...
		if(searchCouList.isEmpty()) {
			return new CourseSchResponse("選課項目中沒有" + reqCouN + "課程");
		}
		...
		for(String TCS : TCSL) {
			...
			for(SelectionSch TCSSC : TCSSeleCouList) {
				...
				if(TCSC.getEndTime().compareTo(Cou.getStartTime()) <= 0
					...
					return new CourseSchResponse("無法修改 預修改的課程(代碼) : " + Cou.getCourseCode() + 
						"與選修該課程的學生(學號) : " + TCS + "所選的課程(代碼) : "
